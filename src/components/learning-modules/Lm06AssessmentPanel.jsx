...
import {
  fetchLm06AssessmentChallenge,
  postLm06AssessmentAnswers,
...
export default function Lm06AssessmentPanel({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLm06AssessmentCopy(lang);
  const rationales =
    LM06_POST_PASS_RATIONALES[locale] || LM06_POST_PASS_RATIONALES.en;
  ...
  const loadChallenge = useCallback(async () => {
    ...
    try {
      const result = await fetchLm06AssessmentChallenge({
        apiBase,
        idToken,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await postLm06AssessmentAnswers({
        apiBase,
          ...
          scoreLabel={
            submitResult.evaluation?.score != null
              ? copy.passedScore(
                  ...
                  submitResult.evaluation.total ?? challenge?.totalQuestions ?? 9
                )
              : null
          }
          xpLabel={
        ...
        <AssessmentFailState
          ...
          scoreLabel={
            submitResult.evaluation?.score != null
              ? copy.failedScore(
                  submitResult.evaluation.score,
                  submitResult.evaluation.total ?? 9
                )
              : null
          }
          lead={failLead}
                ...
                */}
                <legend className="float-left w-full px-4 pt-3">
                  <AssessmentQuestionHeader
                    ...
                    critical={isLm06CriticalQuestion(question.id)}
