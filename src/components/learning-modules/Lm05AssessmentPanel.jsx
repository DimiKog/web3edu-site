...
import {
  fetchLm05AssessmentChallenge,
  postLm05AssessmentAnswers,
...
export default function Lm05AssessmentPanel({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLm05AssessmentCopy(lang);
  const rationales =
    LM05_POST_PASS_RATIONALES[locale] || LM05_POST_PASS_RATIONALES.en;
  ...
  const loadChallenge = useCallback(async () => {
    ...
    try {
      const result = await fetchLm05AssessmentChallenge({
        apiBase,
        idToken,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await postLm05AssessmentAnswers({
        apiBase,
                ...
                */}
                <legend className="float-left w-full px-4 pt-3">
                  <AssessmentQuestionHeader
                    ...
                    critical={isLm05CriticalQuestion(question.id)}
