...
import {
  fetchLm04AssessmentChallenge,
  postLm04AssessmentAnswers,
...
export default function Lm04AssessmentPanel({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLm04AssessmentCopy(lang);
  const rationales =
    LM04_POST_PASS_RATIONALES[locale] || LM04_POST_PASS_RATIONALES.en;
  ...
  const loadChallenge = useCallback(async () => {
    ...
    try {
      const result = await fetchLm04AssessmentChallenge({
        apiBase,
        idToken,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await postLm04AssessmentAnswers({
        apiBase,
                ...
                */}
                <legend className="float-left w-full px-4 pt-3">
                  <AssessmentQuestionHeader
                    ...
                    critical={isLm04CriticalQuestion(question.id)}
