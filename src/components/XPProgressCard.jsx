    ...
    return (
        ...
        `}
            ...
            <p className="text-xs font-semibold text-[#8A57FF] dark:text-[#B799FF]">
                {lang === "gr" ? `${xpPercent ?? 0}% ολοκληρώθηκε` : `${xpPercent ?? 0}% complete`}
            </p>

            {/* Microcopy */}
            <p className="text-[11px] text-slate-700 dark:text-slate-300 italic mt-0.5">
                {lang === "gr" ? "Συνέχισε — ανεβαίνεις γρήγορα επίπεδα!" : "Keep going — you’re leveling fast!"}
            </p>
            ...
            {/* Remaining XP */}
            <p className="text-xs font-medium text-[#4ACBFF] dark:text-[#7FD8FF] mt-2">
                {remainingXp > 0
                    ? lang === "gr"
                        ? `${remainingXp} XP για το επόμενο επίπεδο`
                        : `${remainingXp} XP to next tier`
                    : lang === "gr"
                        ? "Μέγιστο επίπεδο"
                        : "Tier max reached"}
            </p>
            ...
            <p className="text-xs font-semibold text-[#8A57FF] dark:text-[#B799FF]">
                {lang === "gr"
                    ? `${nextTierPercent ?? 0}% προς το επόμενο επίπεδο`
                    : `${nextTierPercent ?? 0}% toward next tier`}
            </p>
        </div>
    );
};
