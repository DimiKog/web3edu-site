...
const panelClassName =
    "relative overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-br from-[#0A0F1A]/95 via-[#111726]/94 to-[#131C2E]/94 shadow-[0_18px_55px_rgba(2,8,23,0.32)] backdrop-blur-xl";
...
export default function ProjectDetail() {
    ...
    useEffect(() => {
        ...
        fetch(`https://web3edu-api.dimikog.org/web3sbt/resolve/${wallet}`, {
            ...
            .then(data => {
                ...
                setUserRole(role);
                setUserTier(tier);
                setIsCompleted(Boolean(data?.metadata?.projectsCompleted?.[projectCompletionId]));
            })
            .catch((err) => {
                if (err.name !== "AbortError") {
                    if (isUserStateUnavailableError(err)) {
                        console.warn("Backend user state temporarily unavailable; preserving project detail progression state.");
                        return;
                    }
                    setUserRole("learner");
    ...
    const handleSubmitAnswer = async () => {
        ...
        try {
            ...
            const res = await fetch(
                "https://web3edu-api.dimikog.org/projects/complete-answer",
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                ...
                <section className="relative overflow-hidden px-4 pb-20 pt-28 sm:px-6">
                    <div className="mx-auto max-w-3xl">
                        <div className={panelClassName}>
                            ...
                            <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-8 text-white">
                                <h1 className="text-2xl font-semibold">{copy.notFound}</h1>
                                <Link
                                    to={isGR ? "/projects-gr" : "/projects"}
                                    className="mt-5 inline-flex rounded-full border border-[#8A57FF]/35 bg-white/[0.04] px-5 py-2 text-sm font-semibold text-white/84 transition hover:border-[#FF67D2]/45 hover:bg-[#8A57FF]/10 hover:text-white"
                                >
                                    {copy.back}
                                </Link>
                            </div>
                    ...
                    <div className="mx-auto max-w-3xl">
                        <div className={panelClassName}>
                            ...
                            <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-8 text-white">
                                ...
                                <p className="mt-4 text-white/72">{copy.unavailableBody}</p>
                                <Link
                                    to={isGR ? "/projects-gr" : "/projects"}
                                    className="mt-5 inline-flex rounded-full border border-[#8A57FF]/35 bg-white/[0.04] px-5 py-2 text-sm font-semibold text-white/84 transition hover:border-[#FF67D2]/45 hover:bg-[#8A57FF]/10 hover:text-white"
                                >
    ...
    return (
        <PageShell>
            <div className="relative overflow-hidden px-4 pb-20 pt-28 text-white sm:px-6">
                <div className="pointer-events-none absolute inset-0">
                    <div className="absolute -top-28 left-1/2 h-[34rem] w-[34rem] -translate-x-1/2 rounded-full bg-[#8A57FF]/24 blur-[170px]" />
                    <div className="absolute top-[18%] right-[-6rem] h-[26rem] w-[26rem] rounded-full bg-[#4ACBFF]/18 blur-[150px]" />
                    <div className="absolute bottom-[10%] left-[-5rem] h-[24rem] w-[24rem] rounded-full bg-[#FF67D2]/14 blur-[140px]" />
                    <div className="absolute inset-0 opacity-[0.03] bg-[linear-gradient(rgba(138,87,255,0.45)_1px,transparent_1px),linear-gradient(90deg,rgba(74,203,255,0.35)_1px,transparent_1px)] bg-[size:54px_54px]" />
                </div>

                <div className="relative mx-auto flex max-w-5xl flex-col gap-8">
                    <Link
                        to={isGR ? "/projects-gr" : "/projects"}
                        className="inline-flex w-fit items-center gap-2 rounded-full border border-[#8A57FF]/35 bg-white/[0.04] px-4 py-2 text-sm font-medium text-white/84 backdrop-blur-md transition hover:border-[#FF67D2]/45 hover:bg-[#8A57FF]/10 hover:text-white"
                    >
                    ...
                    <section className={panelClassName}>
                        ...
                        <div className="relative grid gap-8 rounded-[28px] border border-white/10 bg-gradient-to-br from-[#0A0F1A]/94 via-[#101728]/94 to-[#131D30]/94 lg:grid-cols-[1.15fr_0.85fr]">
                            <div className="p-8 md:p-10">
                                <div className="inline-flex rounded-full border border-[#8A57FF]/40 bg-[#8A57FF]/16 px-4 py-1 text-sm font-semibold text-[#D8C6FF] shadow-[0_0_20px_rgba(138,87,255,0.22)]">
                                    {project.difficulty}
                                </div>
                                <h1 className="mt-5 bg-gradient-to-r from-[#FFFFFF] via-[#FF67D2] to-[#4ACBFF] bg-clip-text text-4xl font-black tracking-tight text-transparent md:text-5xl">
                                    {project.title}
                                </h1>
                            ...
                            <div className="border-t border-white/10 bg-gradient-to-br from-[#141F38]/95 to-[#18243B]/95 p-6 lg:border-l lg:border-t-0 lg:p-8">
                                <img
                                    src={project.image}
                                    alt={project.title}
                    ...
                    <div className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
                        <section className={panelClassName}>
                            ...
                            <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-8 md:p-10">
                                <h2 className="text-2xl font-bold text-white">{copy.description}</h2>
                                ...
                                {project.content && (
                                    <div className="mt-10 space-y-8">
                                        <div>
                                            <h2 className="text-2xl font-bold text-white">{copy.instructions}</h2>
                                        ...
                                        {project.content.txHash && (
                                            <InfoBlock title={copy.txHash}>
                                                <code className="block max-w-full overflow-hidden rounded-2xl border border-[#4ACBFF]/15 bg-[#16213A]/92 px-4 py-3 text-sm text-white/88 break-all break-words">
                                                ...
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="mt-3 inline-flex rounded-full border border-[#4ACBFF]/32 bg-[#4ACBFF]/10 px-4 py-2 text-sm font-semibold text-[#A8EEFF] transition hover:border-[#8A57FF]/38 hover:bg-[#8A57FF]/10 hover:text-white"
                                                >
                                                    {copy.openExplorer}
                                                </a>
                                            </InfoBlock>
                                        ...
                                        {project.content.key && (
                                            <InfoBlock title={copy.key}>
                                                <div>
                                                    <code className="block rounded-2xl border border-[#4ACBFF]/15 bg-[#16213A]/92 px-4 py-3 text-sm text-white/88 break-all">
                                                        {project.content.key}
                                                    </code>
                                                    <button
                                                        onClick={() => copyToClipboard(project.content.key, "key")}
                                                        className="mt-2 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs font-semibold text-white/72 hover:border-[#4ACBFF]/40 hover:text-white"
                                                    >
                                                        {copied === "key"
                                            ...
                                            <InfoBlock title={copy.steps}>
                                                ...
                                                <ul className="space-y-3">
                                                    {normalizedSteps.map((step, index) => (
                                                        <li
                                                            key={`${index}-${step}`}
                                                            className="flex items-start gap-3 rounded-2xl border border-white/10 bg-gradient-to-r from-white/[0.12] to-white/[0.06] px-4 py-4 text-white/82 shadow-sm"
                                                        >
                                                            ...
                                                            <span className="min-w-0 whitespace-normal leading-7 text-white/75 [overflow-wrap:anywhere]">
                                                                {step}
                                                            </span>
                                                        </li>
                                                    ))}
                                            ...
                                            <>
                                                ...
                                                {project.type === "tx-investigation" &&
                                                    ...
                                                    hasCorrectMessageWrongTx && (
                                                        <div className="mt-4 rounded-2xl border border-yellow-400/25 bg-yellow-400/10 px-4 py-3 text-sm text-yellow-200">
                                                            {isGR
                        ...
                        <div className="space-y-8">
                            <section className={panelClassName}>
                                ...
                                <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-8">
                                    <h2 className="text-2xl font-bold text-white">{copy.skills}</h2>
                                    <div className="mt-5 flex flex-wrap gap-3">
                                        {project.skills.map((skill) => (
                                            <span
                                                key={skill}
                                                className="rounded-full border border-[#4ACBFF]/30 bg-gradient-to-r from-[#0F233E] to-[#102946] px-4 py-2 text-sm font-semibold text-[#B7F1FF]"
                                            >
                                                {skill}
                                            </span>
                                        ))}
                            ...
                            <section className={panelClassName}>
                                ...
                                <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-8">
                                    <h2 className="text-2xl font-bold text-white">{copy.submit}</h2>

                                    {!isBuilder && (
                                        <p className="mt-4 rounded-2xl border border-yellow-400/25 bg-yellow-400/10 px-4 py-3 text-sm font-medium text-yellow-200">
                                            {copy.unlock}
                                        </p>
                                    )}
                                    ...
                                    {isBuilder && (
                                        <>
                                            ...
                                            <textarea
                                                ...
                                                className={`mt-5 min-h-[140px] w-full rounded-[24px] px-4 py-4 text-white outline-none transition placeholder:text-white/40 [color-scheme:dark] [-webkit-text-fill-color:white] [caret-color:white] focus:border-white/20 focus:ring-2 focus:ring-white/10 ${isCompleted ? "border-white/5 bg-[#1A243A] opacity-80 cursor-default" : "border-[#4ACBFF]/15 bg-[#16213A]/92"}`}
                                            />
                                            ...
                                            {isCompleted && (
                                                <button
                                                    disabled
                                                    className="mt-5 inline-flex rounded-full bg-white/10 px-6 py-3 text-sm font-semibold text-white/60 cursor-not-allowed"
                                                >
                                                    {isGR ? "✔ Ολοκληρώθηκε" : "✔ Completed"}
                                                </button>
                                            )}
                                            {isCompleted && (
                                                <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/70 space-y-2">
                                                    <h4 className="font-semibold text-white">
                                                        {project.content?.completionTitle}
                                                    </h4>
                                                    <div className="space-y-2">
                                                        {project.content?.completionDetails?.map((item) => (
                                                            <div key={item} className="flex items-start gap-3 rounded-xl border border-white/6 bg-white/[0.02] px-3 py-2">
                                            ...
                                            {isCompleted && (
                                                <div className="mt-4 rounded-2xl border border-[#4ACBFF]/20 bg-[#4ACBFF]/10 p-4 text-sm text-white/70 space-y-2">
                                                    <h4 className="font-semibold text-white">
                                    ...
                                    <Link
                                        to={isGR ? "/projects-gr" : "/projects"}
                                        className="mt-5 inline-flex rounded-full border border-[#8A57FF]/35 bg-white/[0.04] px-5 py-2.5 text-sm font-semibold text-white/84 transition hover:border-[#FF67D2]/45 hover:bg-[#8A57FF]/10 hover:text-white"
                                    >
                                        {copy.back}
                                    </Link>
                                </div>
            ...
            {shouldShowReflectionForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#030712]/78 p-4 backdrop-blur-sm">
                    <div className="relative w-full max-w-xl overflow-hidden rounded-[28px]">
                        <div className="absolute -inset-0.5 rounded-[28px] bg-gradient-to-r from-[#4ACBFF]/28 via-[#8A57FF]/30 to-[#FF67D2]/24 blur-sm" />
                        <div className="relative rounded-[28px] border border-white/10 bg-[#0D1423]/96 p-6 shadow-[0_30px_80px_rgba(2,8,23,0.55)] md:p-8">
                            ...
                            <div className="mt-5 space-y-4">
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full rounded-2xl border border-white/10 bg-[#16213A]/92 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-white/20 focus:ring-2 focus:ring-white/10"
                            ...
                            {reflectionState.error && (
                                <p className="mt-3 text-sm text-[#FFC2EE]">
                                    {reflectionState.error}
                                </p>
                            )}
                            <div className="mt-5 flex flex-wrap gap-3">
                                ...
                                <button
                                    ...
                                    disabled={reflectionState.isSubmitting}
                                    className="inline-flex rounded-full border border-white/12 bg-white/[0.04] px-5 py-2.5 text-sm font-semibold text-white/78 transition hover:border-white/20 hover:bg-white/[0.07] hover:text-white disabled:cursor-not-allowed disabled:opacity-70"
                                >
                                    {copy.feedbackSkip}
                                </button>
                            </div>
...
function MetaBadge({ label, value, tone = "indigo" }) {
    const toneClassName =
        tone === "cyan"
            ? "border border-[#4ACBFF]/35 bg-[#4ACBFF]/14 text-[#A8EEFF]"
            : tone === "pink"
                ? "border border-[#FF67D2]/35 bg-[#FF67D2]/14 text-[#FFC2EE]"
                : "border border-[#8A57FF]/35 bg-[#8A57FF]/16 text-[#D8C6FF]";
...
function getStatusClassName(type) {
    if (type === "success") {
        return "border border-[#4ACBFF]/28 bg-[#4ACBFF]/10 text-[#A8EEFF]";
    }

    if (type === "info") {
        return "border border-[#8A57FF]/28 bg-[#8A57FF]/10 text-[#D8C6FF]";
    }

    return "border border-[#FF67D2]/28 bg-[#FF67D2]/10 text-[#FFC2EE]";
}
