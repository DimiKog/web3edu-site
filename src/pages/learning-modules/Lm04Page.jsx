...
import Lm04ConceptPanel from "../../components/learning-modules/Lm04ConceptPanel.jsx";
...
/**
 * LM04 Interactive Chapter — Keys, Wallets, Identity.
 * @param {{ lang?: "en"|"gr" }} props
 */
export default function Lm04Page({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLmPageCopy(locale, "LM04");
  ...
  const view = getLmPageViewState(progression, locale, "LM04");
    ...
    <PageShell>
      <div className="mx-auto max-w-7xl px-4 pt-24 pb-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        <nav className="mb-5 text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="mx-2">›</span>
          <span className="text-slate-700 dark:text-slate-200">LM04</span>
        </nav>
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)] xl:gap-8">
          <div className="min-w-0 space-y-6">
            <section className="relative isolate overflow-hidden rounded-[1.75rem] border border-slate-300/70 bg-gradient-to-br from-slate-100 via-[#E8EEFF] to-emerald-100/80 px-6 py-7 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] dark:border-slate-500/20 dark:from-slate-950/90 dark:via-slate-900 dark:to-emerald-950/40 sm:px-8 sm:py-8">
              <div className="pointer-events-none absolute -right-16 -top-20 -z-10 h-72 w-72 rounded-full bg-emerald-400/20 blur-3xl dark:bg-emerald-300/10" />
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(200px,0.75fr)] lg:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-semibold text-slate-700 dark:text-emerald-200">
                  ...
                  <p className="mt-6 text-sm font-bold uppercase tracking-[0.25em] text-emerald-700 dark:text-emerald-300">
                    LM04
                  </p>
                  ...
                  <div className="mt-5 max-w-xl border-l-2 border-emerald-400/60 pl-4 text-sm leading-6 text-slate-700 dark:border-emerald-400/40 dark:text-slate-200">
            ...
            <section
              aria-labelledby="lm04-outcomes-title"
            ...
            <LmLearningPath
              ...
              renderEmbed={(row, { lang: embedLang }) => {
                if (row.id === "lm04-interactive-chapter") {
                  return <Lm04ConceptPanel lang={embedLang} />;
                }
                return null;
              }}
            />
