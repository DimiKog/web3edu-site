...
import Lm06ConceptPanel from "../../components/learning-modules/Lm06ConceptPanel.jsx";
import {
  LmApprovedVisual,
  LmHeroMetaMark,
  LmOutcomeMark,
...
import { getLm06ChapterCopy } from "../../content/lm06ChapterLocale.js";
...
/**
 * LM06 Interactive Chapter — Consensus & Block Inclusion.
 * Pedagogical landing page. Does not award XP or change evidence semantics.
 * @param {{ lang?: "en"|"gr" }} props
 */
export default function Lm06Page({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLmPageCopy(locale, "LM06");
  const chapterCopy = getLm06ChapterCopy(locale);
  ...
  const view = getLmPageViewState(progression, locale, "LM06");
    ...
    <PageShell>
      <div className="mx-auto max-w-7xl px-4 pt-24 pb-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        <nav className="mb-5 text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="mx-2">›</span>
          <span className="text-slate-700 dark:text-slate-200">LM06</span>
        </nav>
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)] xl:gap-8">
          <div className="min-w-0 space-y-6">
            <section className="relative isolate overflow-hidden rounded-[1.75rem] border border-slate-300/70 bg-gradient-to-br from-slate-100 via-[#E8EEFF] to-violet-100/80 px-6 py-7 shadow-[0_24px_70px_-45px_rgba(15,23,42,0.45)] dark:border-slate-500/20 dark:from-slate-950/90 dark:via-slate-900 dark:to-violet-950/40 sm:px-8 sm:py-8">
              <div className="pointer-events-none absolute -right-16 -top-20 -z-10 h-72 w-72 rounded-full bg-violet-400/20 blur-3xl dark:bg-violet-300/10" />
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(200px,0.75fr)] lg:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-semibold text-slate-700 dark:text-violet-200">
                    ...
                    {view.presentation ? (
                      <span className="text-slate-600 dark:text-slate-300">
                        {copy.moduleOf(
                          view.presentation.moduleNumber,
                          view.presentation.totalModules
                        )}
                      </span>
                    ) : null}
                  </div>

                  <p className="mt-6 text-sm font-bold uppercase tracking-[0.25em] text-violet-700 dark:text-violet-300">
                    LM06
                  </p>
                  ...
                  <div className="mt-5 max-w-xl border-l-2 border-violet-400/60 pl-4 text-sm leading-6 text-slate-700 dark:border-violet-400/40 dark:text-slate-200">
                    ...
                    <p className="mt-2 font-medium text-slate-800 dark:text-slate-100">
                      <span className="mr-2 font-semibold text-violet-700 dark:text-violet-300">
                        {copy.toLabel}:
                      </span>
                      {view.transitionTo}
                    </p>
              ...
              <dl className="mt-6 grid gap-4 border-t border-slate-300/40 pt-5 dark:border-slate-400/20 sm:grid-cols-3">
            ...
            <section
              aria-labelledby="lm06-outcomes-title"
            ...
            <LmLearningPath
              ...
              lang={locale}
              moduleId="LM06"
              intro={copy.learningPathIntro}
              renderEmbed={(row, { lang: embedLang }) => {
                if (row.id === "lm06-interactive-chapter") {
                  return <Lm06ConceptPanel lang={embedLang} />;
                }
                return null;
              }}
            />
