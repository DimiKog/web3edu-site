...
import Lm03ConceptPanel from "../../components/learning-modules/Lm03ConceptPanel.jsx";
...
/**
 * LM03 Interactive Chapter — From Blockchain Concept to Blockchain Platforms.
 * @param {{ lang?: "en"|"gr" }} props
 */
export default function Lm03Page({ lang = "en" }) {
  const locale = lang === "gr" ? "gr" : "en";
  const copy = getLmPageCopy(locale, "LM03");
  ...
  const view = getLmPageViewState(progression, locale, "LM03");
    ...
    <PageShell>
      <div className="mx-auto max-w-7xl px-4 pt-24 pb-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        <nav className="mb-5 text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="mx-2">›</span>
          <span className="text-slate-700 dark:text-slate-200">LM03</span>
        </nav>
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)] xl:gap-8">
          <div className="min-w-0 space-y-6">
            ...
            <section
              aria-labelledby="lm03-outcomes-title"
            ...
            <LmLearningPath
              ...
              renderEmbed={(row, { lang: embedLang }) => {
                if (row.id === "lm03-separate-dimensions") {
                  return <Lm03ConceptPanel lang={embedLang} panel="dimensions" />;
                }
                if (row.id === "lm03-requirements-to-characteristics") {
                  return <Lm03ConceptPanel lang={embedLang} panel="requirements" />;
                }
                if (row.id === "lm03-platform-comparison") {
                  return <Lm03ConceptPanel lang={embedLang} panel="canvas" />;
                }
                if (row.id === "lm03-foodtrace-revisited") {
                  return <Lm03ConceptPanel lang={embedLang} panel="foodtrace" />;
                }
                return null;
              }}
            />
