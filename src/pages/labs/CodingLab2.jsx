import { useAccount } from "wagmi";
import { useIdentity } from "../../context/useIdentity.js";
import LabTemplate from "./LabTemplate";
import { useLabAutoStartOnce } from "../../hooks/useLabAutoStartOnce.js";
import codingLab01HeroImg from "../../assets/labs/coding1.webp";

const CONTENT = {
    en: {
        title: "Coding Lab 02 — Interact With Your Deployed Counter Contract",
        subtitle:
            "This lab uses the Counter contract you deployed and verified in Coding Lab 01. You will read its current value, send an increment transaction, and verify that the on-chain state changed.",
        conceptualFocusText:
            "You will practice read-only contract calls, state-changing transactions, gas, transaction confirmation, and on-chain state verification — all on the Counter contract you deployed in Coding Lab 01.",
        level: "Beginner",
        estimatedTime: "10–15 minutes",
        tools: [
            "Browser wallet (MetaMask or equivalent)",
            "Besu Edu-Net",
            "Your verified Counter contract from Coding Lab 01",
        ],
        prerequisites: [
            "Coding Lab 01 completed with a verified Counter contract on Besu Edu-Net",
            "Browser wallet connected to Besu Edu-Net",
            "Basic familiarity with read vs write contract calls",
        ],
        interactionPath: "/labs/coding-02/interaction",
        labsOverviewPath: "/labs",
        labels: {
            headerPill: "🧪 Web3Edu · Coding Lab",
            backToLabsLabel: "Back to all labs",
            heroCaption:
                "Reading state is free — changing it requires a signed transaction and gas.",
            conceptualFocus: "Conceptual focus",
            startLab: "Start interaction lab →",
            startLabInteractionHint: "Opens the interactive lab inside Web3Edu",
        },
    },
    gr: {
        title: "Coding Lab 02 — Αλληλεπίδραση με το δικό σου Counter Contract",
        subtitle:
            "Αυτό το lab χρησιμοποιεί το Counter contract που έκανες deploy και επαλήθευσες στο Coding Lab 01. Θα διαβάσεις την τρέχουσα τιμή του, θα στείλεις συναλλαγή increment και θα επιβεβαιώσεις ότι η on-chain κατάσταση άλλαξε.",
        conceptualFocusText:
            "Θα εξασκηθείς σε read-only contract calls, state-changing transactions, gas, επιβεβαίωση συναλλαγής και on-chain state verification — όλα στο Counter contract που έκανες deploy στο Coding Lab 01.",
        level: "Αρχάριο",
        estimatedTime: "10–15 λεπτά",
        tools: [
            "Πορτοφόλι browser (MetaMask ή αντίστοιχο)",
            "Besu Edu-Net",
            "Το επαληθευμένο Counter contract σου από το Coding Lab 01",
        ],
        prerequisites: [
            "Ολοκληρωμένο Coding Lab 01 με επαληθευμένο Counter contract στο Besu Edu-Net",
            "Πορτοφόλι browser συνδεδεμένο στο Besu Edu-Net",
            "Βασική εξοικείωση με read vs write contract calls",
        ],
        interactionPath: "/labs-gr/coding-02/interaction",
        labsOverviewPath: "/labs-gr",
        labels: {
            breadcrumbLabs: "Εργαστήρια",
            backToLabsLabel: "Επιστροφή στα εργαστήρια",
            level: "Επίπεδο",
            estimatedTime: "Εκτιμώμενος χρόνος",
            tools: "Χρησιμοποιούμενα εργαλεία",
            prerequisites: "Προαπαιτούμενα",
            xp: "XP",
            badgeLabel: "Badge",
            startLab: "Έναρξη διαδραστικού lab →",
            startLabInteractionHint: "Ανοίγει το διαδραστικό εργαστήριο στο Web3Edu",
            conceptualFocus: "Εννοιολογική εστίαση",
            heroCaption:
                "Η ανάγνωση κατάστασης είναι δωρεάν — η αλλαγή της απαιτεί υπογεγραμμένη συναλλαγή και gas.",
            headerPill: "🧪 Web3Edu · Coding Lab",
            completedBadge: "✓ Ολοκληρώθηκε",
            checkingStatus: "Έλεγχος κατάστασης ολοκλήρωσης…",
            completedOn: "Ολοκληρώθηκε στις:",
            completionTitle: "Ολοκλήρωση Εργαστηρίου",
            completionDescription:
                "Αφού ολοκληρώσετε το εργαστήριο, διεκδικήστε ολοκλήρωση μέσα στη διαδραστική σελίδα.",
            successMessage: "✔ Η ολοκλήρωση καταγράφηκε επιτυχώς",
            completeInsideInteraction:
                "Ολοκληρώστε τη διαδραστική δραστηριότητα και διεκδικήστε ολοκλήρωση εκεί.",
            walletNotConnectedError: "Το πορτοφόλι δεν είναι συνδεδεμένο",
            labIdMissingError: "Λείπει το αναγνωριστικό του εργαστηρίου",
            backendError: "Αποτυχία καταγραφής ολοκλήρωσης",
        },
    },
};

export default function CodingLab02({ lang = "en" }) {
    const { address } = useAccount();
    const { smartAccount } = useIdentity();
    const t = CONTENT[lang] || CONTENT.en;

    useLabAutoStartOnce({ labId: "coding02", smartAccount, address });

    return (
        <LabTemplate
            labId="coding02"
            title={t.title}
            subtitle={t.subtitle}
            conceptualFocusText={t.conceptualFocusText}
            heroImage={codingLab01HeroImg}
            level={t.level}
            estimatedTime={t.estimatedTime}
            tools={t.tools}
            prerequisites={t.prerequisites}
            interactionPath={t.interactionPath}
            labsOverviewPath={t.labsOverviewPath}
            readmeUrl="https://github.com/DimiKog/web3edu-labs"
            xp={350}
            badge="Contract Interaction Builder"
            labels={t.labels}
        />
    );
}
