import{d as q,r as g,j as e}from"./vendor-react-CTtygi22.js";import{u as Y,X as Z,Y as K,Z as R,_ as L,N as Q,V as H,$ as m,a0 as J,M as ee,P as te,R as O,T as U,a1 as E,a2 as C,a3 as D,U as se}from"./index-C4o_1ueY.js";import{u as re}from"./vendor-web3-DPTBSe4K.js";function ie(){const{address:a}=q(),{address:I}=re(),{owner:S,smartAccount:_}=Y(),[w,p]=g.useState(!0),[v,u]=g.useState(""),[t,z]=g.useState(null);g.useEffect(()=>{if(window.scrollTo(0,0),!a){u("Μη έγκυρη διεύθυνση."),p(!1);return}const r=Z();p(!0),u("");const l=async o=>{const c=await fetch(o);if(!c.ok){const d=await c.json().catch(()=>({}));throw se(c.status,d)}return c.json()};Promise.allSettled([l(`${r}/web3sbt/verify/${a}`),l(K(a,_,I,S))]).then(([o,c])=>{const d=o.status==="fulfilled"?o.value:null,x=c.status==="fulfilled"?c.value:null;if(!d&&!x)throw new Error("Both verify and resolve failed.");const F={...d||{},...x||{},metadata:{...L(d),...L(x)},profile:{...R(d),...R(x)},tokenId:d?.tokenId??d?.token_id??x?.tokenId??x?.token_id??null};F.xp_total=Q(F),z(F),p(!1)}).catch(o=>{if(H(o)){console.warn("Backend user state temporarily unavailable during verify."),u("Τα δεδομένα προφίλ δεν είναι προσωρινά διαθέσιμα. Δοκιμάστε ξανά."),p(!1);return}console.error("Verify page error:",o),u("Αδυναμία φόρτωσης των δεδομένων SBT."),p(!1)})},[a,_,I,S]);const A=t?.metadata??{},B=t?.profile??{},i={...m(A),...m(m(A)?.metadata)},n={...m(B),...m(m(B)?.metadata)},k=(...r)=>{for(const l of r)if(l!=null&&l!=="")return l;return null},f=r=>{if(typeof r=="number"&&Number.isFinite(r))return r;if(typeof r=="string"){const l=Number.parseFloat(r.replace(/[^\d.-]/g,""));if(Number.isFinite(l))return l}return null},V=(i.founder??n.founder)||!1,b=i.edition??n.edition??(V?"Founder Edition":null),N=f(t?.xp_total)??0,$=J(N),{tier:s,overallPercent:W}=$,P=k(i.timeline,n.timeline,t?.timeline,t?.metadata?.timeline,t?.profile?.timeline),G=Array.isArray(P)?P.filter(r=>{const l=String(r?.status||"").toLowerCase(),o=f(r?.progress)??0;return r?.type==="lab"&&(r?.completed===!0||r?.done===!0||r?.verified===!0||!!r?.completedAt||["completed","done","claimed","verified"].includes(l)||o>=100)}).length:null,h=k(i.builderChecklist,n.builderChecklist,t?.builderChecklist,t?.metadata?.builderChecklist),X=h?(f(h?.coreLabs?.completed)??0)+(f(h?.daoLabs?.completed)??0)+(h?.proofOfEscape?.done||h?.proofOfEscape?.completed?1:0):null,M=f(k(i.lessonsCompleted,i.lessons_completed,i.completedLessons,i.completed_lessons,n.lessonsCompleted,n.lessons_completed,n.completedLessons,n.completed_lessons,t?.lessonsCompleted,t?.lessons_completed,t?.completedLessons,t?.completed_lessons,t?.metadata?.lessonsCompleted,t?.metadata?.lessons_completed,t?.profile?.lessonsCompleted,t?.profile?.lessons_completed,X,G))??0,j=a?`https://web3edu.dimikog.org/#/verify-gr/${a}`:"",y=t?.tokenId??null,T=a?ee(a):"";return e.jsx(te,{children:e.jsxs("div",{className:`
        min-h-screen 
