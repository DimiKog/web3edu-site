import{d as $,r as w,j as e}from"./vendor-react-DIuPT6mD.js";import{l as W,k as T,g as M,i as V,p as m,o as q,s as K,P as Q,q as L,r as O,E,B as C,u as D,e as Z}from"./index-Ble26waH.js";function Y(){const{address:l}=$(),[v,p]=w.useState(!0),[k,u]=w.useState(""),[t,z]=w.useState(null);w.useEffect(()=>{if(window.scrollTo(0,0),!l){u("Μη έγκυρη διεύθυνση."),p(!1);return}const s="https://web3edu-api.dimikog.org";p(!0),u("");const a=async o=>{const c=await fetch(o);if(!c.ok){const d=await c.json().catch(()=>({}));throw Z(c.status,d)}return c.json()};Promise.allSettled([a(`${s}/web3sbt/verify/${l}`),a(`${s}/web3sbt/resolve/${l}`)]).then(([o,c])=>{const d=o.status==="fulfilled"?o.value:null,x=c.status==="fulfilled"?c.value:null;if(!d&&!x)throw new Error("Both verify and resolve failed.");const g={...d||{},...x||{},metadata:{...T(d),...T(x)},profile:{...W(d),...W(x)},tokenId:d?.tokenId??d?.token_id??x?.tokenId??x?.token_id??null};g.xp_total=M(g),console.log("XP from backend:",g.xp_total),z(g),p(!1)}).catch(o=>{if(V(o)){console.warn("Backend user state temporarily unavailable during verify."),u("Τα δεδομένα προφίλ δεν είναι προσωρινά διαθέσιμα. Δοκιμάστε ξανά."),p(!1);return}console.error("Verify page error:",o),u("Αδυναμία φόρτωσης των δεδομένων SBT."),p(!1)})},[l]);const I=t?.metadata??{},_=t?.profile??{},i={...m(I),...m(m(I)?.metadata)},n={...m(_),...m(m(_)?.metadata)},N=(...s)=>{for(const a of s)if(a!=null&&a!=="")return a;return null},f=s=>{if(typeof s=="number"&&Number.isFinite(s))return s;if(typeof s=="string"){const a=Number.parseFloat(s.replace(/[^\d.-]/g,""));if(Number.isFinite(a))return a}return null},R=(i.founder??n.founder)||!1,b=i.edition??n.edition??(R?"Founder Edition":null),y=f(t?.xp_total)??0,S=q(y),r=S.tier,B=S.overallPercent,A=N(i.timeline,n.timeline,t?.timeline,t?.metadata?.timeline,t?.profile?.timeline),U=Array.isArray(A)?A.filter(s=>{const a=String(s?.status||"").toLowerCase(),o=f(s?.progress)??0;return s?.type==="lab"&&(s?.completed===!0||s?.done===!0||s?.verified===!0||!!s?.completedAt||["completed","done","claimed","verified"].includes(a)||o>=100)}).length:null,h=N(i.builderChecklist,n.builderChecklist,t?.builderChecklist,t?.metadata?.builderChecklist),G=h?(f(h?.coreLabs?.completed)??0)+(f(h?.daoLabs?.completed)??0)+(h?.proofOfEscape?.done||h?.proofOfEscape?.completed?1:0):null,X=f(N(i.lessonsCompleted,i.lessons_completed,i.completedLessons,i.completed_lessons,n.lessonsCompleted,n.lessons_completed,n.completedLessons,n.completed_lessons,t?.lessonsCompleted,t?.lessons_completed,t?.completedLessons,t?.completed_lessons,t?.metadata?.lessonsCompleted,t?.metadata?.lessons_completed,t?.profile?.lessonsCompleted,t?.profile?.lessons_completed,G,U))??0,j=window.location.href,F=t?.tokenId??null,P=l?K(l):"";return e.jsx(Q,{children:e.jsxs("div",{className:`
        min-h-screen 
...
`,children:"Επαλήθευση Ταυτότητας Web3Edu"}),e.jsx("p",{className:`
        ...
        tracking-wide
    `,children:"Δημόσια επαλήθευση ενός Soulbound Web3Edu Identity Token."})]}),v&&e.jsx("p",{className:"relative z-10 text-slate-600 dark:text-slate-200",children:"Φόρτωση ταυτότητας…"}),k&&!v&&e.jsx("p",{className:"relative z-10 text-red-500 dark:text-red-300",children:k}),!v&&!k&&t&&e.jsxs("div",{className:`
...
`,children:"ΔΗΜΟΣΙΑ ΕΠΑΛΗΘΕΥΣΗ"}),e.jsx("div",{className:"pointer-events-none absolute inset-x-0 top-1/2 h-20 bg-gradient-to-r from-transparent via-white/8 to-transparent opacity-40 blur-xl"}),e.jsxs("div",{className:`relative flex flex-col md:flex-row items-center md:items-start justify-between gap-6 mb-8
                max-w-3xl mx-auto w-full`,children:[e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsxs("div",{className:"relative w-20 h-20 rounded-full flex items-center justify-center shadow-2xl "+(r==="Architect"?"animate-pulse":r==="Builder"?"ring-2 ring-[#33D6FF]/70":"ring-2 ring-[#7F3DF1]/60"),style:L(l,r),children:[e.jsx(O,{address:l}),e.jsxs("span",{className:"absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-black/60 border border-white/40 flex items-center justify-center",children:[r==="Explorer"&&e.jsx(E,{}),r==="Builder"&&e.jsx(C,{}),r==="Architect"&&e.jsx(D,{})]})]}),e.jsxs("div",{children:[e.jsx("p",{className:"text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-white/50",children:"Ταυτότητα Soulbound"}),e.jsx("p",{className:"text-xl font-semibold mt-1 text-slate-900 dark:text-white",children:n.name??"Web3Edu Identity SBT"}),e.jsxs("p",{className:"text-xs text-slate-500 dark:text-white/60 mt-1",children:["Διεύθυνση: ",e.jsx("span",{className:"font-mono",children:P})]}),b&&e.jsx("span",{className:`mt-2 inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-semibold
                                                           bg-gradient-to-r from-[#7F3DF1] via-[#9A5CFF] to-[#FF6FD8]
                                                           text-white shadow-sm`,children:b})]})]}),r&&e.jsxs("div",{className:"flex flex-col items-center md:items-end gap-2",children:[e.jsxs("span",{className:"px-3 py-1 rounded-full text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-md "+(r==="Architect"?"bg-amber-100/90 border-amber-300/70 dark:bg-yellow-300/20 dark:border-yellow-300/40":"bg-slate-100 border-slate-200 dark:bg-white/10 dark:border-white/20"),children:[r==="Explorer"&&e.jsx(E,{}),r==="Builder"&&e.jsx(C,{}),r==="Architect"&&e.jsx(D,{}),r]}),e.jsxs("p",{className:"text-xs text-slate-500 dark:text-white/60",children:["XP: ",y," • Μαθήματα: ",X]})]})]}),e.jsx("div",{className:"mt-6 relative",children:e.jsxs("div",{className:`
