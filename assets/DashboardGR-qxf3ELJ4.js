import{u as pe,r as d,j as e}from"./vendor-react-XZtyZRd7.js";import{s as me,P as ue}from"./index-Ceg9LdUa.js";import{I as he,D as x,F as w,a as I,b as be,X as fe,c as O,L as ge,d as we,e as ke}from"./IdentityCard-53DPjz3n.js";import{u as je}from"./vendor-web3-Dys35WFk.js";import"./identity-icon-MXImXT_G.js";function Le(){const{address:p,isConnected:j}=je(),c=pe(),[r,$]=d.useState(null),[z,v]=d.useState(!1),[X,N]=d.useState(!1),f=d.useRef(null),[m,q]=d.useState(null),[y,W]=d.useState(null),[M,B]=d.useState(!1),[A,J]=d.useState(localStorage.getItem("web3edu-builder-claimed")==="true"),[G,L]=d.useState(!1),U=d.useRef(null),K={tier:"Explorer",xp:0,xpPercent:0,remainingXp:0,nextTierPercent:0,lessonsCompleted:0},V=r&&typeof r=="object"&&!Array.isArray(r)?r:{},C={...K,...V},H=(()=>{const t=C||{},a=m||{},n=[...Array.isArray(t.attributes)?t.attributes:[],...Array.isArray(a.attributes)?a.attributes:[]].some(u=>(u?.trait_type||"").toLowerCase()==="founder"&&(u?.value===!0||String(u?.value).toLowerCase()==="true"));return t.founder===!0||a.founder===!0||t.edition==="Founder Edition"||a.edition==="Founder Edition"||t.role==="Founder"||a.role==="Founder"||n})();d.useEffect(()=>{j||c("/join-gr"),window.scrollTo(0,0)},[j,c]);const k=me(p),Q=t=>{const a=[t?.tokenId,t?.token_id,t?.tokenID,t?.metadata?.tokenId,t?.metadata?.token_id,t?.profile?.tokenId,t?.profile?.token_id,t?.metadata?.metadata?.tokenId,t?.metadata?.metadata?.token_id,t?.profile?.metadata?.tokenId,t?.profile?.metadata?.token_id];for(const s of a)if(s!=null&&s!=="")return s;return null};d.useEffect(()=>{if(!p)return;fetch(`https://web3edu-api.dimikog.org/web3sbt/resolve/${p}`).then(a=>{if(!a.ok)throw new Error(`HTTP ${a.status}`);return a.json()}).then(a=>{const s=a?.metadata?.metadata&&typeof a.metadata.metadata=="object"?a.metadata.metadata:a?.metadata&&typeof a.metadata=="object"?a.metadata:{},n=a?.profile?.metadata&&typeof a.profile.metadata=="object"?a.profile.metadata:a?.profile&&typeof a.profile=="object"?a.profile:{};$({...s,tokenId:Q(a)});const u=o=>{if(typeof o=="string")try{return JSON.parse(o)}catch{return null}return o},T=o=>{const oe=[o?.attributes,o?.attribute,o?.attrs,o?.traits,o?.traits_array,o?.traitsArray,o?.attributes_json,o?.attributesJson];for(const de of oe){const b=u(de);if(Array.isArray(b))return b;if(b&&typeof b=="object")return Object.entries(b).map(([ce,xe])=>({trait_type:ce,value:xe}))}return[]},re=T(s),ae=T(n),h=[...re,...ae],D=n.role||s.role,R=n.specialization||n.speciality||s.specialization||s.speciality,se=h.some(o=>(o.trait_type||"").toLowerCase()==="role"),le=h.some(o=>["specialization","speciality"].includes((o.trait_type||"").toLowerCase()));!se&&D&&h.push({trait_type:"Role",value:D}),!le&&R&&h.push({trait_type:"Specialization",value:R});const ie=n.image&&n.image.trim()!==""?n.image:n.avatar&&n.avatar.trim()!==""?n.avatar:s.image&&s.image.trim()!==""?s.image:s.avatar&&s.avatar.trim()!==""?s.avatar:null,ne={...s,...n,name:n.name||s.name||k||"Web3Edu Identity",image:ie,attributes:h};q(ne),W(new Date)}).catch(a=>{console.error("Failed to fetch metadata:",a)})},[p,k]),d.useEffect(()=>{if(!r||typeof r.xp!="number")return;let t;return f.current==null||r.xp>f.current&&(N(!0),t=setTimeout(()=>N(!1),1200)),f.current=r.xp,()=>{t&&clearTimeout(t)}},[r]),d.useEffect(()=>{const t=r?.tier;if(t)try{localStorage.setItem("web3edu-tier",t)}catch(a){console.error("Failed to persist tier in localStorage:",a)}},[r?.tier]),d.useEffect(()=>{r?.tier&&(r.tier==="Builder"&&!A&&B(!0),U.current=r.tier)},[r?.tier,A]);const Y={type:"guide",title:"Ξεκίνα εδώ — Οδηγός Web3Edu",slug:"start-here-gr",why:"Αυτός ο σύντομος οδηγός εξηγεί πώς λειτουργεί το Web3Edu και σε βοηθά να επιλέξεις το επόμενο βήμα.",estimatedTime:5},g=r&&typeof r.recommendedNext=="object"?r.recommendedNext:null,E=g&&(g.title||g.slug),l=E?g:Y,F=!E,_=!!l?.builderRequired,S=l?.slug?.endsWith("-gr")?l.slug.replace(/-gr$/,""):l?.slug,Z={lab01:"Lab 01 — Πορτοφόλια & Web3 Ταυτότητες","wallets-keys":"Lab 01 — Πορτοφόλια & Web3 Ταυτότητες",lab02:"Lab 02 — Κρυπτογραφημένα Μηνύματα",lab03:"Lab 03 — Υπογραφή Μηνυμάτων & Ιδιοκτησία",lab04:"Lab 04 — Συναλλαγές & Gas",lab05:"Lab 05 — Έξυπνα Συμβόλαια & Κατάσταση",lab06:"Lab 06 — Συναίνεση & Οριστικότητα",dao01:"DAO Lab 01 — Διακυβέρνηση & Ψηφοφορία","dao-01":"DAO Lab 01 — Διακυβέρνηση & Ψηφοφορία",dao02:"DAO Lab 02 — Μοντέλα Διακυβέρνησης & Δυναμικές Ισχύος","dao-02":"DAO Lab 02 — Μοντέλα Διακυβέρνησης & Δυναμικές Ισχύος","proof-of-escape":"Lab 01 — Proof of Escape",poe:"Lab 01 — Proof of Escape"},ee=(r?.timeline||[]).map(t=>{if(!t||t.type!=="lab")return t;const a=String(t.id||t.slug||"").toLowerCase(),s=Z[a];return s?typeof t.title=="object"&&t.title!==null?{...t,title:{...t.title,gr:t.title.gr||s}}:{...t,title:s}:t}),i=r?.builderChecklist||null,[P,te]=d.useState(r?.tier&&r.tier!=="Explorer");return e.jsx(ue,{children:e.jsxs("div",{className:`
        ...
        `,children:[e.jsx("div",{className:"flex justify-center mb-3",children:e.jsxs("div",{className:"relative",children:[e.jsx("img",{src:m.image||"/icons/web3edu-identity.png",alt:"avatar",className:`
                    ...
                    `})]})}),e.jsxs("h2",{className:"text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center justify-center gap-3",children:["Καλώς ήρθες πάλι, ",m.name||k," 👋",r?.tier&&e.jsx("span",{className:`
            ...
            `,children:r.tier})]}),y&&e.jsxs("p",{className:"text-xs text-slate-500 dark:text-slate-400 mt-1",children:["🕒 Συγχρονίστηκε πριν ",Math.max(1,Math.floor((Date.now()-y.getTime())/1e3))," δευτ • Ταυτότητα Web3Edu"]}),e.jsx("p",{className:"mt-2 text-sm text-slate-600 dark:text-slate-300",children:"Κάνε mint την Ταυτότητα Web3Edu για να αποθηκεύσεις τη μάθησή σου on-chain."}),e.jsx("div",{className:`
            ...
            `})]}),M&&e.jsx("div",{className:"relative z-10 w-full max-w-4xl mx-auto mb-10 px-4",children:e.jsxs("div",{className:`
                        ...
                        `,children:[e.jsx("h2",{className:"text-2xl font-extrabold text-white mb-3",children:"🏗️ Ξεκλειδώθηκε το επίπεδο Builder"}),e.jsxs("p",{className:"text-sm text-slate-200 mb-6",children:["Ολοκλήρωσες τις βασικές απαιτήσεις και ξεκλείδωσες το επίπεδο",e.jsx("span",{className:"font-semibold text-purple-300",children:" Builder "}),". Πλέον μπορείς να συμμετέχεις σε προχωρημένες διαδρομές διακυβέρνησης."]}),G?e.jsxs("div",{className:`
                                ...
                                backdrop-blur-xl shadow-xl text-slate-900 dark:text-slate-100
                                hover:scale-[1.005] hover:shadow-2xl transition-all duration-500
                            `,icon:e.jsx(I,{className:"w-5 h-5 text-white"}),children:e.jsx("p",{className:"text-sm font-mono text-slate-700 dark:text-slate-300 break-all",children:p??"—"})}),e.jsx(x,{title:"Βαθμίδα",className:`
                            ...
                            `,icon:e.jsx(be,{className:"w-5 h-5 text-white"}),children:e.jsxs("div",{className:"flex flex-col items-start gap-3 cursor-pointer",onClick:()=>v(!0),children:[e.jsxs("div",{className:`
                            ...
                            hover:scale-[1.005] hover:shadow-2xl transition-all duration-500
                        `,icon:e.jsx(w,{className:"w-5 h-5 text-white"}),children:e.jsx(fe,{xp:r?.xp??0,xpPercent:r?.xpPercent??0,remainingXp:r?.remainingXp??0,nextTierPercent:r?.nextTierPercent??0,tier:r?.tier??"Explorer",xpLeveledUp:X,lang:"gr"})}),e.jsxs(x,{title:"Ενέργειες",className:`
                            rounded-2xl border border-indigo-300/30 dark:border-indigo-700/30
                            bg-gradient-to-br from-white/95 via-indigo-50/75 to-slate-100/90
                            dark:from-[#0E1426]/90 dark:via-[#0B1020]/85 dark:to-[#070C18]/90
                            dark:border-white/10 backdrop-blur-xl shadow-xl text-slate-900 dark:text-slate-100
...
`,children:"Ξεκίνα εδώ (οδηγός)"})})]})]}),e.jsxs(x,{title:"Σήματα",className:`
                            ...
                            dark:border-white/10 backdrop-blur-xl shadow-xl text-slate-900 dark:text-slate-100
                            hover:scale-[1.005] hover:shadow-2xl transition-all duration-500
                        `,icon:e.jsx(w,{className:"w-5 h-5 text-white"}),children:[e.jsx("p",{className:"text-sm text-slate-700 dark:text-slate-200 mb-4 leading-relaxed",children:"Όλες οι διακρίσεις και τα σήματα που κερδίζεις θα εμφανίζονται εδώ καθώς προχωράς."}),r&&r.badges?.length>0?e.jsx("div",{className:"flex flex-wrap gap-2",children:r.badges.map((t,a)=>{let s=w;const n=typeof t=="string"?t.toLowerCase():t?.label?.toLowerCase?.()||t?.en?.toLowerCase?.()||t?.gr?.toLowerCase?.()||"";return n.includes("wallet")&&(s=I),n.includes("lesson")&&(s=we),n.includes("quiz")&&(s=ke),e.jsxs("span",{className:`
                                                    inline-flex items-center gap-2 
                                                    px-3 py-1 rounded-full 
