import{d as H,r as y,j as e}from"./vendor-react-XZtyZRd7.js";import{s as K,P as Q,g as V,h as L,E as D,B as P,i as S}from"./index-CJhuLKjR.js";function ee(){const{address:o}=H(),[E,j]=y.useState(!0),[I,_]=y.useState(""),[t,M]=y.useState(null);y.useEffect(()=>{if(window.scrollTo(0,0),!o){_("Invalid address."),j(!1);return}const n="https://web3edu-api.dimikog.org";j(!0),_("");const i=async d=>{const f=await fetch(d);if(!f.ok)throw new Error(`HTTP ${f.status}`);return f.json()};Promise.allSettled([i(`${n}/web3sbt/verify/${o}`),i(`${n}/web3sbt/resolve/${o}`)]).then(([d,f])=>{const h=d.status==="fulfilled"?d.value:null,u=f.status==="fulfilled"?f.value:null;if(!h&&!u)throw new Error("Both verify and resolve failed.");const W=l=>l?.metadata?.metadata&&typeof l.metadata.metadata=="object"?l.metadata.metadata:l?.metadata&&typeof l.metadata=="object"?l.metadata:{},R=l=>l?.profile?.metadata&&typeof l.profile.metadata=="object"?l.profile.metadata:l?.profile&&typeof l.profile=="object"?l.profile:{},q={...h||{},...u||{},metadata:{...W(h),...W(u)},profile:{...R(h),...R(u)},tokenId:h?.tokenId??h?.token_id??u?.tokenId??u?.token_id??null};M(q),j(!1)}).catch(d=>{console.error("Verify page error:",d),_("Could not load SBT data."),j(!1)})},[o]);const g=t?.metadata??{},w=t?.profile??{},r=g?.metadata&&typeof g.metadata=="object"?g.metadata:g,s=w?.metadata&&typeof w.metadata=="object"?w.metadata:w,p=(...n)=>{for(const i of n)if(i!=null&&i!=="")return i;return null},c=n=>{if(typeof n=="number"&&Number.isFinite(n))return n;if(typeof n=="string"){const i=Number.parseFloat(n.replace(/[^\d.-]/g,""));if(Number.isFinite(i))return i}return null},O=(r.founder??s.founder)||!1,v=r.edition??s.edition??(O?"Founder Edition":null),a=r.tier??s.tier??"Explorer",x=c(p(r.xp,r.xp_total,r.totalXp,r.total_xp,s.xp,s.xp_total,s.totalXp,s.total_xp,t?.xp,t?.xp_total,t?.totalXp,t?.total_xp,t?.metadata?.xp,t?.metadata?.xp_total,t?.metadata?.totalXp,t?.metadata?.total_xp,t?.profile?.xp,t?.profile?.xp_total,t?.profile?.totalXp,t?.profile?.total_xp))??0,N=c(p(r.xpPercent,r.xp_percent,r.progress,r.progressPercent,r.progress_percent,r.nextTierPercent,r.next_tier_percent,r.completionPercent,r.completion_percent,s.xpPercent,s.xp_percent,s.progress,s.progressPercent,s.progress_percent,s.nextTierPercent,s.next_tier_percent,s.completionPercent,s.completion_percent,t?.xpPercent,t?.xp_percent,t?.progress,t?.progressPercent,t?.progress_percent,t?.nextTierPercent,t?.next_tier_percent,t?.completionPercent,t?.completion_percent))??0,F=c(p(r.remainingXp,r.remaining_xp,s.remainingXp,s.remaining_xp,t?.remainingXp,t?.remaining_xp,t?.metadata?.remainingXp,t?.metadata?.remaining_xp)),X=N>0&&N<=1?N*100:N;let m=Math.max(0,Math.min(100,X));m<=0&&x>0&&(F!==null&&F>=0?m=Math.max(1,Math.min(100,x/(x+F)*100)):(m=Math.max(1,Math.min(100,x/6e3*100)),a==="Architect"&&(m=Math.max(m,85))));const z=Math.max(0,Math.min(100,x/7e3*100)),A=Math.max(m,z),B=p(r.timeline,s.timeline,t?.timeline,t?.metadata?.timeline,t?.profile?.timeline),$=Array.isArray(B)?B.filter(n=>{const i=String(n?.status||"").toLowerCase(),d=c(n?.progress)??0;return n?.type==="lab"&&(n?.completed===!0||n?.done===!0||n?.verified===!0||!!n?.completedAt||["completed","done","claimed","verified"].includes(i)||d>=100)}).length:null,b=p(r.builderChecklist,s.builderChecklist,t?.builderChecklist,t?.metadata?.builderChecklist),U=b?(c(b?.coreLabs?.completed)??0)+(c(b?.daoLabs?.completed)??0)+(b?.proofOfEscape?.done||b?.proofOfEscape?.completed?1:0):null,G=c(p(r.lessonsCompleted,r.lessons_completed,r.completedLessons,r.completed_lessons,s.lessonsCompleted,s.lessons_completed,s.completedLessons,s.completed_lessons,t?.lessonsCompleted,t?.lessons_completed,t?.completedLessons,t?.completed_lessons,t?.metadata?.lessonsCompleted,t?.metadata?.lessons_completed,t?.profile?.lessonsCompleted,t?.profile?.lessons_completed,U,$))??0,k=`https://web3edu.dimikog.org/#/verify/${o}`,C=t?.tokenId??null,T=o?K(o):"";return e.jsx(Q,{children:e.jsxs("div",{className:`
        min-h-screen 
...
`,children:"Web3Edu Identity Verification"}),e.jsx("p",{className:`
        ...
        tracking-wide
    `,children:"Public verification of a Soulbound Web3Edu Identity Token."})]}),E&&e.jsx("p",{className:"relative z-10 text-slate-600 dark:text-slate-200",children:"Loading identity…"}),I&&!E&&e.jsx("p",{className:"relative z-10 text-red-500 dark:text-red-300",children:I}),!E&&!I&&t&&e.jsxs("div",{className:`
...
`,children:"PUBLIC VERIFICATION"}),e.jsx("div",{className:"pointer-events-none absolute inset-x-0 top-1/2 h-20 bg-gradient-to-r from-transparent via-white/8 to-transparent opacity-40 blur-xl"}),e.jsxs("div",{className:`relative flex flex-col md:flex-row items-center md:items-start justify-between gap-6 mb-8
                max-w-3xl mx-auto w-full`,children:[e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsxs("div",{className:"relative w-20 h-20 rounded-full flex items-center justify-center shadow-2xl "+(a==="Architect"?"animate-pulse":a==="Builder"?"ring-2 ring-[#33D6FF]/70":"ring-2 ring-[#7F3DF1]/60"),style:V(o,a),children:[e.jsx(L,{address:o}),e.jsxs("span",{className:"absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-black/60 border border-white/40 flex items-center justify-center",children:[a==="Explorer"&&e.jsx(D,{}),a==="Builder"&&e.jsx(P,{}),a==="Architect"&&e.jsx(S,{})]})]}),e.jsxs("div",{children:[e.jsx("p",{className:"text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-white/50",children:"Soulbound Identity"}),e.jsx("p",{className:"text-xl font-semibold mt-1 text-slate-900 dark:text-white",children:s.name??"Web3Edu Identity SBT"}),e.jsxs("p",{className:"text-xs text-slate-500 dark:text-white/60 mt-1",children:["Wallet: ",e.jsx("span",{className:"font-mono",children:T})]}),v&&e.jsx("span",{className:`mt-2 inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-semibold
                                                           bg-gradient-to-r from-[#7F3DF1] via-[#9A5CFF] to-[#FF6FD8]
                                                           text-white shadow-sm`,children:v})]})]}),a&&e.jsxs("div",{className:"flex flex-col items-center md:items-end gap-2",children:[e.jsxs("span",{className:"px-3 py-1 rounded-full text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-md "+(a==="Architect"?"bg-amber-100/90 border-amber-300/70 dark:bg-yellow-300/20 dark:border-yellow-300/40":"bg-slate-100 border-slate-200 dark:bg-white/10 dark:border-white/20"),children:[a==="Explorer"&&e.jsx(D,{}),a==="Builder"&&e.jsx(P,{}),a==="Architect"&&e.jsx(S,{}),a]}),e.jsxs("p",{className:"text-xs text-slate-500 dark:text-white/60",children:["XP: ",x," • Lessons: ",G]})]})]}),e.jsx("div",{className:"mt-6 relative",children:e.jsxs("div",{className:`
