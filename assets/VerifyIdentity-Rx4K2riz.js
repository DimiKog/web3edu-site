import{d as q,r as g,j as e}from"./vendor-react-CTtygi22.js";import{u as K,H as Q,I as H,J as T,K as V,y as J,F as Y,L as m,M as Z,x as ee,P as te,C as L,D as O,N as F,O as I,Q as C,E as se}from"./index-C5pHNW8O.js";import{u as re}from"./vendor-web3-Lztbhc-M.js";function ie(){const{address:a}=q(),{address:D}=re(),{owner:S,smartAccount:A}=K(),[v,p]=g.useState(!0),[w,u]=g.useState(""),[t,U]=g.useState(null);g.useEffect(()=>{if(window.scrollTo(0,0),!a){u("Invalid address."),p(!1);return}const r=Q();p(!0),u("");const l=async o=>{const c=await fetch(o);if(!c.ok){const d=await c.json().catch(()=>({}));throw se(c.status,d)}return c.json()};Promise.allSettled([l(`${r}/web3sbt/verify/${a}`),l(H(a,A,D,S))]).then(([o,c])=>{const d=o.status==="fulfilled"?o.value:null,x=c.status==="fulfilled"?c.value:null;if(!d&&!x)throw new Error("Both verify and resolve failed.");const E={...d||{},...x||{},metadata:{...V(d),...V(x)},profile:{...T(d),...T(x)},tokenId:d?.tokenId??d?.token_id??x?.tokenId??x?.token_id??null};E.xp_total=J(E),U(E),p(!1)}).catch(o=>{if(Y(o)){console.warn("Backend user state temporarily unavailable during verify."),u("Profile data is temporarily unavailable. Please try again."),p(!1);return}console.error("Verify page error:",o),u("Could not load SBT data."),p(!1)})},[a,A,D,S]);const _=t?.metadata??{},B=t?.profile??{},i={...m(_),...m(m(_)?.metadata)},n={...m(B),...m(m(B)?.metadata)},N=(...r)=>{for(const l of r)if(l!=null&&l!=="")return l;return null},f=r=>{if(typeof r=="number"&&Number.isFinite(r))return r;if(typeof r=="string"){const l=Number.parseFloat(r.replace(/[^\d.-]/g,""));if(Number.isFinite(l))return l}return null},z=(i.founder??n.founder)||!1,b=i.edition??n.edition??(z?"Founder Edition":null),k=f(t?.xp_total)??0,M=Z(k),{tier:s,overallPercent:P}=M,W=N(i.timeline,n.timeline,t?.timeline,t?.metadata?.timeline,t?.profile?.timeline),$=Array.isArray(W)?W.filter(r=>{const l=String(r?.status||"").toLowerCase(),o=f(r?.progress)??0;return r?.type==="lab"&&(r?.completed===!0||r?.done===!0||r?.verified===!0||!!r?.completedAt||["completed","done","claimed","verified"].includes(l)||o>=100)}).length:null,h=N(i.builderChecklist,n.builderChecklist,t?.builderChecklist,t?.metadata?.builderChecklist),G=h?(f(h?.coreLabs?.completed)??0)+(f(h?.daoLabs?.completed)??0)+(h?.proofOfEscape?.done||h?.proofOfEscape?.completed?1:0):null,X=f(N(i.lessonsCompleted,i.lessons_completed,i.completedLessons,i.completed_lessons,n.lessonsCompleted,n.lessons_completed,n.completedLessons,n.completed_lessons,t?.lessonsCompleted,t?.lessons_completed,t?.completedLessons,t?.completed_lessons,t?.metadata?.lessonsCompleted,t?.metadata?.lessons_completed,t?.profile?.lessonsCompleted,t?.profile?.lessons_completed,G,$))??0,j=`https://web3edu.dimikog.org/#/verify/${a}`,y=t?.tokenId??null,R=a?ee(a):"";return e.jsx(te,{children:e.jsxs("div",{className:`
        min-h-screen 
